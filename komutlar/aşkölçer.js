...
let clark = yankee[Math.floor(Math.random() * yankee.length)]
let ritmik = msg.mentions.members.first()
     if (!ritmik) return msg.channel.send(':heart: Kimi Sevdiğini Etiketle?') 
  msg.channel.send(`${msg.author}, ${ritmik} İle Olan Aşkını ${clark}.`)
    }
