...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.reply("⚔️ Oynamak istediğin kişiyi etiketlemelisin!")
  
  if (opponent.bot) return message.reply('⚔️ Botlar ile oynayamazsın!');
  if (opponent.id === message.author.id) return message.reply('⚔️ Kendin ile düello Atamazsın');
		if (this.fighting.has(message.channel.id)) return message.reply('⚔️ Kanal başına sadece bir düello meydana gelebilir.');
		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(`⚔️ Düello kabul edilmedi...`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					...
					if (!turn.size) {
						await message.reply(`⚔️ Üzgünüm ama, süre doldu!`);
						reset();
						continue;
					}
				...
				if (choice === 'saldır') {
					...
					await message.reply('⚔️ Ne yapmak istediğini anlamadım.');
				}
			}
			this.fighting.delete(message.channel.id);
